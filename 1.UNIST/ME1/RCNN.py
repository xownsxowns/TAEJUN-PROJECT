...
for train, test in kfold.split(re_data, label):
    ...
    model = Sequential()
    model.add(Conv1D(filters=64, kernel_size=3 , input_shape=(1000, 29), activation='relu', padding='same'))
    ...
    model.fit(re_data[train], label[train], epochs=100, batch_size=20)
    # evaluate the model
