...
eeg = np.reshape(eeg,(500,1000,29))
...
print(model.summary())
model.fit(eeg, label, epochs=100, batch_size=20, verbose=2)

# with a Sequential model
get_3rd_layer_output = K.function([model.layers[0].input],
                                  [model.layers[1].output])
layer_output = get_3rd_layer_output([eeg])[0]
np.shape(layer_output)
latent = StandardScaler().fit_transform(np.transpose(layer_output)).transpose()

for i in range(10):
    # label이 1인 index 찾기
    index = label[label['label']==i].index
    ...
    def plot_generated_images(epoch, mnist_num, generator, latent, examples=1, dim=(1,1), figsize=(10,10)):
        ...
        for k in range(generated_images.shape[0]):
            ...
            plt.axis('off')
        plt.tight_layout()
        plt.savefig('all_latent_gan_generated_image_' + str(mnist_num) + '_epoch_%d.png' % epoch)
    ...
    for e in range(1, epochs + 1):
        ...
        for _ in tqdm(range(batch_count)):
            ...
            noise_eeg = np.concatenate((noise, latent[index,:]), axis=1)
