...
for isub in range(30,60):
    ...
    df = pd.DataFrame(cm)
    filename = 'P300_Result_balanced_bsmote_confusion_' + str(isub + 1) + '_train.csv'
    df.to_csv(filename)
...
for isub in range(14):
    ...
    df = pd.DataFrame(cm)
    filename = 'P300_Result_BS_balanced_bsmote_confusion_' + str(isub + 1) + '_train.csv'
    df.to_csv(filename)
...
filename = 'P300_Result_SVM_bsmote_balanced.csv'
df.to_csv(filename)

df2 = pd.DataFrame(train_score)
filename = 'P300_Result_SVM_bsmote_balanced_trainscore.csv'
df2.to_csv(filename)
