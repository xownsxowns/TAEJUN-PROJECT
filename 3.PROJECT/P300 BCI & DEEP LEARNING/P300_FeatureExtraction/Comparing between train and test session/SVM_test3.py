...
for isub in range(30,60):
    ...
    tar_data_test = list()
    tar_label_test = list()
    nontar_data_test = list()
    nontar_label_test = list()

    for i in range(15):
        target = data2['ERP'][:,150:,data['target'][i][0]-1,i]
        tar_data_test.append(target)
        tar_label_test.append(1)

        for j in range(4):
            if j == (data2['target'][i][0]-1):
                continue
            else:
                nontar_data_test.append(data2['ERP'][:,150:,j,i])
                nontar_label_test.append(0)

    tar_data = np.concatenate((tar_data, tar_data_test))
    nontar_data = np.concatenate((nontar_data, nontar_data_test))
    tar_label = np.concatenate((tar_label, tar_label_test))
    nontar_label = np.concatenate((nontar_label, nontar_label_test))
...
filename = 'P300_Result_comparing_session3.csv'
df.to_csv(filename)

df2 = pd.DataFrame(train_score)
filename = 'P300_Result_comparing_session3_trainscore.csv'
df2.to_csv(filename)