...
for repeat_num in range(1,6):
    ...
    for isub in range(30,60):
        sm = RandomUnderSampler(random_state=5)
        print(isub)
        ...
        model_name = '//192.168.1.181/office/[04] 연구원개인문서함/이태준/[9] 졸업논문/model/undersampling/RUS/SAE/model_SAE_rus_t' + str(repeat_num) + '_train' + str(isub + 1) + '.h5'
        ...
        filename = 'C:/Users/jhpark/Documents/GitHub/TAEJUN PROJECT/[3]PROJECT/P300 BCI & DEEP LEARNING/P300_sampling/UnderSampling/CONFUSION/RUS/' \
                    'SAE_rus_t' + str(repeat_num) + '_confusion_' + str(isub + 1) + '.csv'
        df.to_csv(filename)
    ...
    for isub in range(14):
        ...
        model_name = '//192.168.1.181/office/[04] 연구원개인문서함/이태준/[9] 졸업논문/model/undersampling/RUS/SAE/model_BS_SAE_rus_t' + str(repeat_num) + '_train' + str(isub + 1) + '.h5'
        ...
        df = pd.DataFrame(confusion_mat)
        filename = 'C:/Users/jhpark/Documents/GitHub/TAEJUN PROJECT/[3]PROJECT/P300 BCI & DEEP LEARNING/P300_sampling/UnderSampling/CONFUSION/RUS/' \
                   'SAE_BS_rus_t' + str(repeat_num) + '_confusion_' + str(isub + 1) + '.csv'
        df.to_csv(filename)
    ...
    filename = 'P300_Result_SAE_rus_t' + str(repeat_num) + '.csv'
    df.to_csv(filename)

    df2 = pd.DataFrame(train_score)
    filename = 'P300_Result_SAE_rus_t' + str(repeat_num) + '_trainscore.csv'
    df2.to_csv(filename)