...
## Train
path = 'E:/[1] Experiment/[1] BCI/P300LSTM/Epoch_data/Epoch/Sub' + str(test_sub+45) + '_EP_training.mat'
data = io.loadmat(path)
...
for k in range(train_tl_data.shape[1]):
    train_tl_data[:, k, :] = scalers[i].transform(train_tl_data[:, k, :])
## Train (Transfer learning)
new_input = model.input
hidden_layer = model.layers[-2].output
dense1 = Dense(40, activation='relu')(hidden_layer)
new_output = Dense(1, activation='sigmoid')(dense1)
model2 = Model(new_input, new_output)
model2.fit(train_tl_data, train_tl_label, epochs=200, batch_size=5)
...
filename = 'P300_Result_CNN_TL10.csv'
df.to_csv(filename)