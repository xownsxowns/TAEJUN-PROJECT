...
for isub in range(30,60):
    ...
    train_vali_data = np.concatenate((after_tar_data, after_ntar_data))
    ...
    vali_data = np.expand_dims(vali_data, axis = 2)

    model = Sequential()
    model.add(Conv1D(filters=64, kernel_size=10 , input_shape=(np.shape(train_data)[1],1), activation='relu'))
    ...
    for i in range(ntest):
        ...
        for j in range(4):
            ...
            new_test_data = np.expand_dims(new_test_data, axis=2)
            prob = model.predict_proba(new_test_data)
...
filename = 'P300_Result_CNN_ANOVA.csv'
df.to_csv(filename)