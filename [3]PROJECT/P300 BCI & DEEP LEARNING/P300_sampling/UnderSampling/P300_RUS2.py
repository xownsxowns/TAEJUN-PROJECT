...
for repeat_num in range(1,11):
    for isub in range(30,60):
        ...
        model_name = 'E:/[9] 졸업논문/model/undersampling/model_CNN_rus_t' + str(repeat_num) + '_train' + str(isub + 1) + '.h5'
    ...
    for isub in range(14):
        ...
        model_name = 'E:/[9] 졸업논문/model/undersampling/model_BS_CNN_rus_t' + str(repeat_num) + '_train' + str(isub + 1) + '.h5'
        model.save(model_name)
        ## classifier
        ## prob로 하지 않고 그냥 predict로 했을 때
        training_score = accuracy_score(train_label, model.predict_classes(train_data))
    ...
    filename = 'P300_Result_CNN_rus_t' + str(repeat_num) + '.csv'
    df.to_csv(filename)

    df2 = pd.DataFrame(train_score)
    filename = 'P300_Result_CNN_rus__t' + str(repeat_num) + 'trainscore.csv'
    df2.to_csv(filename)

    df3 = pd.DataFrame(train_score_prob)
    filename = 'P300_Result_CNN_rus_t' + str(repeat_num) + '_trainscore_prob.csv'
    df3.to_csv(filename)
