...
for isub in range(30,60):
    ...
    model_name = 'E:/[9] 졸업논문/model/oversampling/model_CNN2_svmsmote_train' + str(isub + 1) + '.h5'
...
for isub in range(14):
    ...
    model_name = 'E:/[9] 졸업논문/model/oversampling/model_BS_CNN2_svmsmote_train' + str(isub + 1) + '.h5'
    model.save(model_name)
    ## classifier
    ## prob로 하지 않고 그냥 predict로 했을 때
    training_score = accuracy_score(train_label, model.predict_classes(train_data))
...
filename = 'P300_Result_CNN2_svmsmote.csv'
df.to_csv(filename)

df2 = pd.DataFrame(train_score)
filename = 'P300_Result_CNN2_svmsmote_trainscore.csv'
df2.to_csv(filename)

df3 = pd.DataFrame(train_score_prob)
filename = 'P300_Result_CNN2_svmsmote_trainscore_prob.csv'
df3.to_csv(filename)