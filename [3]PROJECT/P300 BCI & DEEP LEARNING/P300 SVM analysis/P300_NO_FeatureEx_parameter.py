...
for isub in range(30,60):
    ...
    grid_clf = GridSearchCV(SVC(probability=True, kernel='sigmoid', gamma='auto'), params_grid)
    clf = grid_clf.fit(new_train_data, train_label)
    cm = confusion_matrix(train_label, clf.predict(new_train_data))

    df = pd.DataFrame(cm)
    filename = 'P300_Result_SVM_opt_confusion_' + str(isub + 1) + '_train.csv'
    df.to_csv(filename)
    #
...
for isub in range(14):
    ...
    df = pd.DataFrame(cm)
    filename = 'P300_Result_BS_SVM_opt_confusion_' + str(isub + 1) + '_train.csv'
    df.to_csv(filename)
...
filename = 'P300_opt_Result_NO.csv'
df.to_csv(filename)

df2 = pd.DataFrame(train_score)
filename = 'P300_opt_Result_no_trainscore.csv'
df2.to_csv(filename)
