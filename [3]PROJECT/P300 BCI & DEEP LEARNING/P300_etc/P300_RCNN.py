...
from keras.layers import Dense, Dropout, Flatten, Conv1D, MaxPooling1D, LSTM, SimpleRNN
...
for isub in range(60):
    ...
    model = Sequential()
    model.add(Conv1D(filters=64, kernel_size=3 , input_shape=(nlen, nch), activation='relu', padding='same'))
...
filename = 'P300_RCNN Result_RNN.csv'
df.to_csv(filename)