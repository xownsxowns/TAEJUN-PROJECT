...
for isub in range(60):
    ...
    # Batch normalization은 activation function 후, dropout layer 전
    model = Sequential()
    model.add(Conv1D(filters=32, kernel_size=10 , input_shape=(nlen, nch), padding='same'))
...
filename = 'P300 Result_DCNN_BN.csv'
df.to_csv(filename)