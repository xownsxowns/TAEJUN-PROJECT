...
for isub in range(30,60):
    ...
    for i in range(ntrain):
        target = data2['ERP'][:,150:,data2['target'][i][0]-1,i]
        ...
        for j in range(4):
            if j == (data2['target'][i][0]-1):
                ...
                nontar_data.append(data2['ERP'][:,150:,j,i])
...
filename = 'P300_Result_comparing_session1.csv'
df.to_csv(filename)

df2 = pd.DataFrame(train_score)
filename = 'P300_Result_comparing_session1_trainscore.csv'
df2.to_csv(filename)
