...
for repeat_num in range(1, 6):
    ...
    for isub in range(30, 60):
        ...
        model_name = 'E:/[9] 졸업논문/model/feature_extraction/CNN_2d(full ch)/model_CNN2D_fullch_t' + str(repeat_num) + '_train' + str(
            isub + 1) + '.h5'
        ...
        filename = 'C:/Users/jhpark/Documents/GitHub/Python_project/[3]PROJECT/P300 BCI & DEEP LEARNING/P300_FeatureExtraction/CONFUSION/' \
                   '/P300_Result_CNN_2D_fullch_confusion_' + str(isub + 1) + '_t' + str(repeat_num) + '.csv'
        df.to_csv(filename)
    ...
    for isub in range(14):
        ...
        model_name = 'E:/[9] 졸업논문/model/feature_extraction/CNN_2d(full ch)/model_BS_CNN2D_fullch_t' + str(repeat_num) + '_train' + str(
            isub + 1) + '.h5'
        model.save(model_name)
        ...
        df = pd.DataFrame(confusion_mat)
        filename = 'C:/Users/jhpark/Documents/GitHub/Python_project/[3]PROJECT/P300 BCI & DEEP LEARNING/P300_FeatureExtraction/CONFUSION/' \
                   '/P300_Result_BS_CNN_2D_fullch_confusion_' + str(isub + 1) + '_t' + str(repeat_num) + '.csv'
        df.to_csv(filename)
    ...
    filename = 'P300_Result_CNN_2d_fullch_t' + str(repeat_num) + '.csv'
    df.to_csv(filename)

    df2 = pd.DataFrame(train_score)
    filename = 'P300_Result_CNN_2d_fullch_t' + str(repeat_num) + '_trainscore.csv'
    df2.to_csv(filename)