...
for repeat_num in range(1,11):
    for isub in range(30,60):
        ...
        output_tensor = Dense(K, activation='sigmoid', kernel_regularizer=l2(0.01))(x)
        ...
        model_name = 'E:/[9] 졸업논문/model/model_resnet50_t' + str(repeat_num) + '_train' + str(isub + 1) + '.h5'
    ...
    for isub in range(14):
        ...
        model_name = 'E:/[9] 졸업논문/model/model_BS_resnet50_t' + str(repeat_num) + '_train' + str(isub + 1) + '.h5'
        resnet50.save(model_name)
    ...
    filename = 'P300_Result_DCNN_resnet_t' + str(repeat_num) + '.csv'
    df.to_csv(filename)

    df2 = pd.DataFrame(train_score)
    filename = 'P300_Result_DCNN_resnet_t' + str(repeat_num) + '_trainscore.csv'
    df2.to_csv(filename)

    df3 = pd.DataFrame(train_score_prob)
    filename = 'P300_Result_DCNN_resnet_t' + str(repeat_num) + '_trainscore_prob.csv'
    df3.to_csv(filename)