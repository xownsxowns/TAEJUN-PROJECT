...
for isub in range(30,60):
    print(isub+1)
    path = 'D:/[10] 저널&학회 투고/Sensors/Preprocessed_FIR_wo_ASR/Epoch/Sub' + str(isub+1) + '_EP_training.mat'
    ...
    del_new_train_data = new_train_data[~np.isnan(new_train_data).any(axis=1)]
    del_train_label = train_label[~np.isnan(new_train_data).any(axis=1)]

    clf = SVC(probability=True, kernel='sigmoid', gamma='auto')
    clf.fit(del_new_train_data, del_train_label)
    cm = confusion_matrix(del_train_label, clf.predict(del_new_train_data))

    df = pd.DataFrame(cm)
    filename = 'P300_Result_threshold_confusion_' + str(isub + 1) + '_train.csv'
    df.to_csv(filename)
    #
    ## prob로 하지 않고 그냥 predict로 했을 때
    training_score = accuracy_score(del_train_label, clf.predict(del_new_train_data))
    train_score.append(training_score)

    ## Test
    path = 'D:/[10] 저널&학회 투고/Sensors/Preprocessed_FIR_wo_ASR/Epoch/Sub' + str(isub+1) + '_EP_test.mat'
    ...
    for i in range(ntest):
        ...
        for j in range(4):
            ...
            del_new_test_data = new_test_data[~np.isnan(new_test_data).any(axis=1)]
            if del_new_test_data.size == 0:
                continue
            prob = clf.predict_proba(del_new_test_data)
...
for isub in range(14):
    ...
    df = pd.DataFrame(cm)
    filename = 'P300_Result_BS_threshold_confusion_' + str(isub + 1) + '_train.csv'
    df.to_csv(filename)
...
filename = 'P300_threshold_Result_NO.csv'
df.to_csv(filename)

df2 = pd.DataFrame(train_score)
filename = 'P300_threshold_Result_no_trainscore.csv'
df2.to_csv(filename)
