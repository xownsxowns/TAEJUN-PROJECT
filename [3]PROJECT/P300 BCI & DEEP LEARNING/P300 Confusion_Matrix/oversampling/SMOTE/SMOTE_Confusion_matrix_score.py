...
path = 'C:/Users/jhpark/Documents/GitHub/Python_project/[3]PROJECT/P300 BCI & DEEP LEARNING/P300_sampling/OverSampling/result/CONFUSION/SMOTE/'

############ DOORLOCK ###############################
total_sum_cnn_doorlock = np.zeros((2,2))
for isub in range(31,46):
    file = path + 'CNN_fullch_smote_total_confusion_' + str(isub) + '.csv'
...
for isub in range(1,15):
    file = path + 'CNN_BS_fullch_smote_total_confusion_' + str(isub) + '.csv'
    result = pd.read_csv(file, header=0, index_col=0)
...
filename = 'SMOTE_Confusion matrix.csv'
df.to_csv(filename)