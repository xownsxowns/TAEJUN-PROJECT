...
for repeat_num in range(1,2):
    for isub in range(30,60):
        ros = TomekLinks(random_state=5)
        model_name = 'E:/[9] 졸업논문/model/undersampling/Tomek/CNN Full ch/model_CNN_tomek_t' + str(repeat_num) + '_train' + str(isub + 1) + '.h5'
        ...
        filename = 'C:/Users/jhpark/Documents/GitHub/Python_project/[3]PROJECT/P300 BCI & DEEP LEARNING/P300_sampling/UnderSampling/CONFUSION/Tomek/' \
                   'CNN_fullch_tomek_t' + str(repeat_num) + '_confusion_' + str(isub+1) + '.csv'
        df.to_csv(filename)
    ...
    for isub in range(14):
        ros = TomekLinks(random_state=5)
        model_name = 'E:/[9] 졸업논문/model/undersampling/Tomek/CNN Full ch/model_BS_CNN_tomek_t' + str(repeat_num) + '_train' + str(isub + 1) + '.h5'
        model = load_model(model_name)
        ...
        df = pd.DataFrame(confusion_mat)
        filename = 'C:/Users/jhpark/Documents/GitHub/Python_project/[3]PROJECT/P300 BCI & DEEP LEARNING/P300_sampling/UnderSampling/CONFUSION/Tomek/' \
                   'CNN_BS_fullch_tomek_t' + str(repeat_num) + '_confusion_' + str(isub+1) + '.csv'
        df.to_csv(filename)
